...
import { Fuel, ShieldAlert, LogIn } from 'lucide-react';
...
const FuelLogin = () => {
  const { loginFuelUser } = useContext(AppContext);
  const navigate = useNavigate();

  const [email, setEmail] = useState('citizen@fuel.com');
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await loginFuelUser(email, password);
    setLoading(false);
    if (res.success) {
      if (res.role === 'admin') {
        navigate('/admin');
      } else if (res.role === 'pump') {
        navigate('/pump');
      } else {
  ...
  const handleGoogleLogin = async () => {
    setLoading(true);
    const res = await loginFuelUser('citizen@fuel.com', 'password123');
    setLoading(false);
    if (res.success) {
      navigate('/fuel/dashboard');
    } else {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '3rem 1.5rem' }}>
      <motion.div 
        ...
        style={{ width: '100%', maxWidth: '450px', padding: '2.5rem', border: '1px solid rgba(6, 182, 212, 0.25)', boxShadow: '0 8px 32px 0 rgba(6, 182, 212, 0.1)' }}
      >
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h2 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#ffffff' }}>Fuel Portal Login</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginTop: '0.25rem' }}>
            Verify credentials to access smart quota allocation.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group" style={{ marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              ...
              <a href="#forgot" style={{ fontSize: '0.75rem', color: '#06b6d4', textDecoration: 'none', fontWeight: 500 }} onClick={(e) => { e.preventDefault(); alert("Verification email sent to " + email); }}>
                Forgot Password?
              </a>
            </div>
            <input 
        ...
        <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '10px', backdropFilter: 'blur(8px)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <button
              ...
              className="btn btn-secondary"
              style={{ padding: '0.35rem 0.5rem', fontSize: '0.75rem', textAlign: 'left', display: 'flex', justifyContent: 'space-between', border: '1px solid rgba(6, 182, 212, 0.15)', background: 'rgba(6, 182, 212, 0.05)', color: '#ffffff' }}
            >
              <span>👑 Gov Admin</span>
            ...
            <button
              type="button"
              onClick={() => { setEmail('pump@ceypetco.com'); setPassword('pump123'); }}
              className="btn btn-secondary"
              style={{ padding: '0.35rem 0.5rem', fontSize: '0.75rem', textAlign: 'left', display: 'flex', justifyContent: 'space-between', border: '1px solid rgba(6, 182, 212, 0.15)', background: 'rgba(6, 182, 212, 0.05)', color: '#ffffff' }}
            >
              <span>⛽ Pump Operator</span>
              <span style={{ opacity: 0.6 }}>pump@ceypetco.com</span>
            </button>
            <button
              type="button"
              onClick={() => { setEmail('citizen@fuel.com'); setPassword('password123'); }}
              className="btn btn-secondary"
              style={{ padding: '0.35rem 0.5rem', fontSize: '0.75rem', textAlign: 'left', display: 'flex', justifyContent: 'space-between', border: '1px solid rgba(6, 182, 212, 0.15)', background: 'rgba(6, 182, 212, 0.05)', color: '#ffffff' }}
            >
              <span>👤 Fuel Citizen</span>
              <span style={{ opacity: 0.6 }}>citizen@fuel.com</span>
            </button>
          </div>
        </div>
        ...
        <p style={{ textAlign: 'center', fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '2rem' }}>
          ...
          <Link to="/fuel/register" style={{ color: '#06b6d4', textDecoration: 'none', fontWeight: 600 }}>
            Register Vehicle
          </Link>
        </p>
      </motion.div>
