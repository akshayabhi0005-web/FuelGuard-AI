...
const LpgRegister = () => {
  const { registerLpgUser } = useContext(AppContext);
  ...
  const [formData, setFormData] = useState({
    ...
    state: '',
    consumerNumber: '',
    preferredDistributor: 'Gas Point Corp',
    agreeToTerms: false
  });
  ...
  const distributors = [
    'Gas Point Corp', 
    'Super Gas Distributors', 
    'Lanka Gas Dealers', 
    'Elite LPG Associates'
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await registerLpgUser({
      ...
      consumerNumber: formData.consumerNumber.toUpperCase(),
      preferredDistributor: formData.preferredDistributor
    });
  ...
  return (
    <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '3rem 1.5rem' }}>
      <motion.div 
        ...
        style={{ width: '100%', maxWidth: '650px', padding: '2.5rem', border: '1px solid rgba(249, 115, 22, 0.25)', boxShadow: '0 8px 32px 0 rgba(249, 115, 22, 0.1)' }}
      >
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h2 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#ffffff' }}>LPG Consumer Registration</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginTop: '0.25rem' }}>
            Register your domestic address details to qualify for cylinder booking access.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div>
              <div className="form-group">
                ...
                <input 
                  ...
                  onChange={handleChange}
                  placeholder="e.g. 123 Main St, Garden City"
                  required
                  disabled={loading || success}
                />
              </div>
              ...
              <div className="form-group">
                <label className="form-label">Consumer Number (Optional)</label>
                <input 
                  type="text" 
                  name="consumerNumber"
                  className="form-input" 
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', marginTop: '1rem', marginBottom: '1.5rem', textAlign: 'left' }}>
            ...
            <label htmlFor="agreeToTerms" style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', cursor: 'pointer' }}>
              I agree to the FuelGuard LPG terms, consent to address checks for safety zoning, and verify my gas blue book documentation.
            </label>
          </div>
          ...
          <button 
            ...
            style={{ width: '100%', padding: '0.85rem', background: 'linear-gradient(135deg, #ea580c 0%, #a855f7 100%)', boxShadow: '0 4px 15px rgba(249, 115, 22, 0.2)' }}
            disabled={loading || success}
          >
            {loading ? (
              ...
              <>
                Register LPG Account <ArrowRight size={16} />
              </>
            )}
          </button>
        </form>
        ...
        <p style={{ textAlign: 'center', fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '1.5rem' }}>
          ...
          <Link to="/lpg/login" style={{ color: '#f97316', textDecoration: 'none', fontWeight: 600 }}>
            Login here
          </Link>
        </p>
      </motion.div>
